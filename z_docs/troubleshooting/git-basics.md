...
🛠️ Troubleshooting: Monitoring Stack (Prometheus & Grafana)
인프라의 '눈' 역할을 하는 모니터링 모듈(05_monitoring) 구축 중 발생한 기술적 이슈와 해결 방안을 기록합니다.

1. Unsupported block type (Helm set Block Error)
현상: helm_release 리소스 내에서 set 블록을 정의할 때 Unsupported block type 에러가 발생하며 플랜(Plan)이 중단됨.

원인: 테라폼의 특정 버전이나 Helm Provider 버전에서 set을 블록 형태(set { ... })로 처리하는 방식에 엄격한 문법 검사가 적용됨.

해결: * Provider 업그레이드: terraform init -upgrade를 실행하여 Helm Provider를 최신 안정 버전으로 갱신.

문법 명시화: 블록 형태가 아닌 동적 리스트 형태(dynamic "set" { ... })를 검토하거나, values.yaml 파일을 별도로 생성하여 templatefile() 함수로 주입하는 방식으로 구조 개선.

2. Helm Provider의 Kubernetes 연결 컨텍스트 오류
현상: Blocks of type "kubernetes" are not expected here 또는 클러스터 연결 실패 발생.

원인: Helm Provider가 어느 쿠버네티스 클러스터에 배포해야 할지 모르는 상태이거나, provider "helm" 내부 선언 방식이 구형 문법임.

해결: ~/.kube/config를 명시적으로 참조하도록 설정하여 로컬 컨텍스트와 동기화함.

Terraform
provider "helm" {
  kubernetes {
    config_path = "~/.kube/config" # 로컬 인증 정보를 명시적으로 전달
  }
}
3. 변수 입력 자동화 (Input Automation)
현상: terraform apply 실행 시마다 cluster_name 등 반복되는 변수를 수동으로 입력해야 하는 번거로움 발생.

해결: 프로젝트 루트에 terraform.tfvars 파일을 생성하여 값을 고정함.

주의: 이 파일은 로컬 설정용이므로 깃허브에 올릴 때는 샘플 파일(terraform.tfvars.example)만 올리고 실제 파일은 .gitignore에 등록할 것.
